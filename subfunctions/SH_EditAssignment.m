function strNewAssignment = SH_EditAssignment(strTitle,strAssignment)
	...
	%split assignment
	strS = getFlankedBy(strAssignment,'S','X');
	intFieldSize = numel(strS);
	intS = str2double(strS);
	...
	vecAssignment = [intS intX intY intZ intC];
	...
	handleEdit.ptrTextTiling = uitext(hEditGui,'Style','text','String','Tiling:',...
		'VerticalAlignment','middle','Units','normalized','FontSize',12,'Position',[0.3 0.6 0.4 0.1]);
	
	%X
	handleEdit.ptrTextX = uitext(hEditGui,'Style','text','String','X location',...
		'HorizontalAlignment','left','VerticalAlignment','middle','Units','normalized','FontSize',10,'Position',[dblStartX 0.5 0.2 0.1]);
	...
	handleEdit.ptrButtonAccept = uicontrol(hEditGui,'Style','pushbutton','String','Accept',...
		'HorizontalAlignment','center','Units','normalized','FontSize',12,'Position',[0.25 0.1 0.5 0.15],...
		'Callback',@UIE_Accept);
